...
if [ ! -d "${VSCODE_SERVER_ROOT}/bin" ]; then
    echo "VS Code server not installed" >&2
    exit 0
fi

# LOCK for multiple remote ssh connection
LOCK_DIR="${VSCODE_SERVER_ROOT}/vscode-server-patch.lock.d"
mkdir -p "$VSCODE_SERVER_ROOT" 2>/dev/null || true
